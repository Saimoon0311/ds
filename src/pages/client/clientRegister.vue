    ...
    <div class="hello container">
        ...
        <Form @submit="submitData" class="p-2 px-md-5 m-md-3" :validation-schema="schema" v-slot="{errors}">
            ...
            <p>
                ...
                <router-link  to="/login" class="already btn-light text-dark btn">Already have an account?</router-link ></p>
        </Form>
    </div>
</div>
</div>
...
export default {
    data() {
        const schema = yup.object().shape({
            first_name: yup.string().max(20, 'First name maximum limit is 20 characters.').required('Please enter your first name.'),
            last_name: yup.string().max(35, 'Last name maximum limit is 35 characters.').required('Please enter your last name.'),
